...
const Layout = () => {
  const { searchResult, loading, isSearching } = useSelector(
    (state) => state.movie
  );
  ...
  return (
    <div className="container h-full w-screen grid grid-cols-2">
          ...
          <div className="">
            {searchResult.map((episode) => (
              <MovieContainer
                ...
                title={episode.title}
                onClick={() => onClickHadel(episode.episode_id)}
              />
            ))}
          </div>
        ) : (
      ...
      {movie ? (
        <div className="border-2 border-gray-200 p-8 col-span-1">
          <h2 className="font-semibold text-2xl">{movie.title}</h2>

          {/* Here I formated the opening_crawl  */}
          <p
            className="py-4 antialiased text-gray-500"
